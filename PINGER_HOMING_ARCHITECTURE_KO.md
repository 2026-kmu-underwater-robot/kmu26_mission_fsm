...
kmu26_mission_fsm/src/pinger_homing
  위 추정 결과 + /odometry/filtered + /mavros/state
    -> 능동 위치 추정 및 접근 RC
    -> /control/pinger/rc_override

kmu26_mission_fsm/src/vision_buoy
...
```bash
vcs import src < src/kmu26_mission_fsm/hydrophone_fork.repos
rosdep install --from-paths src --ignore-src -r -y
colcon build --packages-select \
  audio_common_msgs audio_capture hit25_auv_ros2_msg \
  auv_buoy_vision_control kmu26_mission_fsm
...
기존 `kmu26_auv_web_gui`의 **Pinger Homing** 탭도 같은 전용 launch를 실행한다. GUI의 Live

...
private:
    ...
    const MissionStatus & status, const RobotPose & pose) const
  {
    ...
    label << std::fixed << std::setprecision(2)
          << "dist " << det.distance << "m  conf " << det.confidence;
    add_text(
  ...
  void publish_markers()
  {
    ...
    const std::string text = read_text_file(status_path_);
    const MissionStatus status = parse_status(text);
    const bool yolo_fresh = latest_yolo_wall_s_ > 0.0 && (stamp.seconds() - latest_yolo_wall_s_) <= 2.5;
    const YoloStatus yolo = yolo_fresh ? parse_yolo_status(latest_yolo_text_) : YoloStatus{};
    ...
    if (!status.valid) {
      add_text(
        ...
        "Mission RViz\nwaiting for mission_fsm_status.json", 0.35,
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<MissionRvizVisualizer>());
  rclcpp::shutdown();
  return 0;
}
